...
int __weak rockchip_wifi_mac_addr(unsigned char *buf)
{
	return -ENODEV;
}
...
{
	bool mac_set = false;
	const char *mac_source = "unknown";

	RPU_INFO_UMACIF("RK915: ========== MAC Address Initialization ==========\n");

	/* Priority 1: Module parameter (manual override) */
	if (mac_addr && strlen(mac_addr) > 0) {
		if (parse_mac_addr_string(mac_addr, vif_macs[0]) == 0) {
			if (is_valid_ether_addr(vif_macs[0])) {
				RPU_INFO_UMACIF("RK915: MAC address from module parameter: %s\n", mac_addr);
				mac_set = true;
				mac_source = "module parameter";
			} else {
				RPU_ERROR_UMACIF("RK915: Invalid MAC address from module parameter: %s\n", mac_addr);
			}
		} else {
			RPU_ERROR_UMACIF("RK915: Failed to parse MAC address: %s\n", mac_addr);
		}
	}
	...
	if (!mac_set) {
		if (rockchip_wifi_mac_addr(vif_macs[0]) == 0 && is_valid_ether_addr(vif_macs[0])) {
			...
			mac_set = true;
			mac_source = "Rockchip BSP vendor storage";
		} else {
			RPU_INFO_UMACIF("RK915: Rockchip BSP vendor storage not available, using random MAC\n");
		}
	}
	...
	/* Print final MAC addresses */
	RPU_INFO_UMACIF("RK915: --------------------------------------------------\n");
	RPU_INFO_UMACIF("RK915: Final MAC Address Configuration:\n");
	RPU_INFO_UMACIF("RK915:   MAC Source: %s\n", mac_source);
	RPU_INFO_UMACIF("RK915:   VIF 0 MAC:  %pM\n", vif_macs[0]);
	RPU_INFO_UMACIF("RK915:   VIF 1 MAC:  %pM\n", vif_macs[1]);
	RPU_INFO_UMACIF("RK915: ==================================================\n");
}
